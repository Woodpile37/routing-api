...
export async function getV2CandidatePools({
  ...
  if (
    ...
    (tokenOut.symbol == 'WETH' || tokenOut.symbol == 'WETH9' || tokenOut.symbol == 'ETH')
  ) {
  ...
  for (const subgraphPool of subgraphPoolsSorted) {
    ...
    foundTopByEthQuoteTokenPool = topByEthQuoteTokenPool.length >= topNEthQuoteToken
  ...
  const printV2SubgraphPoolAddress = (s: V2SubgraphPool) => `${s.token0.id}/${s.token1.id}`
  ...
  CONTEXT['V2 GetCandidatePools'] = {
    ...
    poolsFoundByAddress: {
      ...
      topByTVLUsingTokenInSecondHops: topByTVLUsingTokenInSecondHops.map(printV2SubgraphPoolAddress),
      topByTVLUsingTokenOutSecondHops: topByTVLUsingTokenOutSecondHops.map(printV2SubgraphPoolAddress),
